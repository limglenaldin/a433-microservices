...
docker build -t karsajobs-ui:latest .                                   # Build image from Dockerfile
docker tag karsajobs-ui:latest ghcr.io/$username/karsajobs-ui:latest    # Change image name to GitHub Package format

echo $pat | docker login ghcr.io -u $username --password-stdin          # Login to GitHub Package
docker push ghcr.io/$username/karsajobs-ui:latest                       # Push image to GitHub Package
